        ...
        for i in range(0, len(string)):
            ...
            if char == Tokenizer.OPENPAREN:
                ...
                if len(curr_value) > 0:
                    ...
                    curr_value = Value()
                self.pushToken(char)
            elif char in numbers or (char == '-' and string[i + 1] in numbers and len(curr_value) == 0 and string[
                i - 1] != Tokenizer.CLOSEPAREN):
        ...
        while token is not None:
            ...
            if token == Tokenizer.OPENPAREN:
                ...
                if curr_value is None:
                    curr_value = token
                    if (
                            tokenizer.peekToken() is None or tokenizer.peekToken() == Tokenizer.CLOSEPAREN) and prev_op is not None:
...
class Operation(Node):
    ...
    def factor(self):
        ...
        if isinstance(self.left, Operation) and type(self.left) == type(
                self.right) and self.left.weight - self.weight == 1:
            # Get grandchildren
            llgc = self.left.left
